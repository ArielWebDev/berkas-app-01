...
const WorkflowActionsImproved: React.FC<WorkflowActionsProps> = ({ pinjaman, user, onSuccess }) => {
    const [loading, setLoading] = useState(false);
    const [alert, setAlert] = useState<Alert>({ type: 'info', message: '', show: false });
    ...
    const showModal = (type: 'catatan' | 'reason' | 'decision', title: string, placeholder: string, onConfirm: (value: string, decision?: string) => void) => {
    ...
    const closeModal = () => {
        setModal({ show: false, type: null, title: '', placeholder: '', onConfirm: () => {} });
        setInputValue('');
        setDecision('');
    };
    ...
    const handleWorkflowAction = async (action: string, data: any = {}) => {
        ...
        try {
            ...
            if (response.ok && result.success !== false) {
                ...
                const errorMessage = result.message || result.error || `${action} gagal`;
    ...
    const canTake = () => {
        ...
        switch (pinjaman.status) {
            ...
            case 'diperiksa':
                ...
                return user.role === 'admin_kredit' && pinjaman.admin_kredit_id === user.id;
            case 'dianalisis':
    ...
    const canReturn = () => {
        return pinjaman.locked_at && pinjaman.locked_by === user.id &&
               (pinjaman.status === 'diperiksa' || pinjaman.status === 'dianalisis' || pinjaman.status === 'siap_diputuskan');
    };
    ...
    const getWorkflowAnalysis = () => {
        ...
        // Analisis untuk Take
        if (pinjaman.status === 'diajukan' && user.role === 'admin_kredit' && !pinjaman.locked_at) {
            analysis.push('✅ Admin kredit bisa TAKE berkas baru (diajukan)');
        } else if (pinjaman.status === 'diperiksa' && user.role === 'admin_kredit' && pinjaman.admin_kredit_id === user.id && !pinjaman.locked_at) {
            analysis.push('✅ Admin kredit bisa TAKE berkas diperiksa (assigned)');
        } else if (pinjaman.status === 'dianalisis' && user.role === 'analis' && pinjaman.analis_id === user.id && !pinjaman.locked_at) {
            analysis.push('✅ Analis bisa TAKE berkas dianalisis (assigned)');
        } else if (pinjaman.status === 'siap_diputuskan' && user.role === 'pemutus' && pinjaman.pemutus_id === user.id && !pinjaman.locked_at) {
            analysis.push('✅ Pemutus bisa TAKE berkas siap_diputuskan (assigned)');
        } else {
            analysis.push('❌ Tidak bisa TAKE berkas - cek status/role/assignment/lock');
        }
        
        // Analisis untuk Advance/Return
        if (pinjaman.status === 'diperiksa' && user.role === 'admin_kredit' && pinjaman.locked_by === user.id) {
            analysis.push('✅ Admin kredit bisa ADVANCE/RETURN berkas diperiksa');
        } else if (pinjaman.status === 'dianalisis' && user.role === 'analis' && pinjaman.locked_by === user.id) {
            analysis.push('✅ Analis bisa ADVANCE/RETURN berkas dianalisis');
        } else if (pinjaman.status === 'siap_diputuskan' && user.role === 'pemutus' && pinjaman.locked_by === user.id) {
            analysis.push('✅ Pemutus bisa DECIDE/RETURN berkas siap_diputuskan');
        } else {
            analysis.push('❌ Tidak bisa ADVANCE/RETURN - berkas tidak dikunci oleh user ini');
        }
        
        // Debug data
        analysis.push(`📋 Data: Status=${pinjaman.status}, Role=${user.role}, UserID=${user.id}`);
        analysis.push(`🔒 Lock: LockedBy=${pinjaman.locked_by}, LockedAt=${pinjaman.locked_at}`);
        analysis.push(`👥 Assignment: AdminKredit=${pinjaman.admin_kredit_id}, Analis=${pinjaman.analis_id}, Pemutus=${pinjaman.pemutus_id}`);
    ...
    const AlertComponent = () => {
        ...
        return (
            <div className={`p-4 mb-4 border rounded-lg ${alertClasses[alert.type]}`}>
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
                <h3 className="text-lg font-semibold mb-4 text-gray-800">Aksi Workflow</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    
                    {/* Debug Info - akan dihapus nanti */}
                    <div className="col-span-full bg-gray-100 p-4 rounded-lg mb-4 text-xs">
                        <strong>DEBUG INFO:</strong><br/>
                        Status: {pinjaman.status}<br/>
                        User Role: {user.role}<br/>
                        Locked At: {pinjaman.locked_at || 'null'}<br/>
                        Locked By: {pinjaman.locked_by || 'null'}<br/>
                        User ID: {user.id}<br/>
                        Can Take: {canTake() ? 'YES' : 'NO'}<br/>
                        Can Advance: {canAdvance() ? 'YES' : 'NO'}<br/>
                        Can Release: {canRelease() ? 'YES' : 'NO'}<br/>
                        Can Return: {canReturn() ? 'YES' : 'NO'}<br/>
                    ...
                    {canRelease() && (
                        <button
                            ...
                            disabled={loading}
                            className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                        >
                    ...
                    {canAdvance() && (
                        <button
                            ...
                            disabled={loading}
                            className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                        >
                    ...
                    {canReturn() && (
                        <button
                            ...
                            disabled={loading}
                            className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                        >
                    ...
                    {canDecide() && (
                        <button
                            onClick={() => {
                                ...
                                const reason = decision ? 'Disetujui' : prompt('Alasan penolakan:');
                                if (reason) {
                                    handleWorkflowAction('finalDecision', { status, reason });
                                }
                            }}
                            disabled={loading}
                            className="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                        >
                ...
                <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
                    <div className="flex items-center">
                        ...
                        <p className="text-sm text-blue-800">
                            <strong>Notifikasi WhatsApp:</strong> Setiap aksi workflow akan mengirim notifikasi otomatis ke admin/analis/pemutus di tahap berikutnya.
                        </p>
                    </div>
                </div>
            </div>
