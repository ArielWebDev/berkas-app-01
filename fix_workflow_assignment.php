...
echo "=== FIXING WORKFLOW ASSIGNMENT ===\n";

// Fix pinjaman dengan status dianalisis yang tidak punya analis_id
...
foreach ($pinjaman as $p) {
    ...
    if ($analis) {
        ...
        $p->locked_by = null; // Unlock agar bisa diambil
        $p->locked_at = null;
        $p->save();
...
// Fix pinjaman dengan status siap_diputuskan yang tidak punya pemutus_id
$pinjaman = Pinjaman::where('status', 'siap_diputuskan')
    ->whereNull('pemutus_id')
    ->get();

echo "\nFound " . $pinjaman->count() . " pinjaman with status 'siap_diputuskan' but no pemutus_id\n";
...
// Reset semua locked berkas yang tidak dalam status final
$resetCount = Pinjaman::whereNotNull('locked_by')
    ->whereNotIn('status', ['disetujui', 'ditolak', 'dikembalikan'])
    ->update([
        'locked_by' => null,
        'locked_at' => null
    ]);
